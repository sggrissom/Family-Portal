...
export const allProvidersOff: AuthProviders = { google: false, apple: false };

export async function loadProviders(): Promise<AuthProviders> {
  try {
...
export function anyProvider(providers: AuthProviders): boolean {
  return providers.google || providers.apple;
}
...
function startUrl(provider: string, inviteCode?: string): string {
  const url = `/api/login/${provider}`;
  return inviteCode ? `${url}?code=${encodeURIComponent(inviteCode)}` : url;
}

export const OAuthButtons = ({ providers, disabled, inviteCode }: OAuthButtonsProps) => (
  <>
    {providers.google && (
      <button
        className="btn btn-oauth btn-google"
        disabled={disabled}
        onClick={() => (window.location.href = startUrl("google", inviteCode))}
      >
        <GoogleIcon />
    ...
    {providers.apple && (
      <button
        ...
        disabled={disabled}
        onClick={() => (window.location.href = startUrl("apple", inviteCode))}
      >
        <AppleIcon />
        Continue with Apple
      </button>
