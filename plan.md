...
- [ ] **`www.familyrecord.app` fails TLS.** The name has an A record pointing at the server, Caddy has no site block claiming it, and so it never obtains a certificate: HTTP 308s to HTTPS and the handshake dies with `tlsv1 alert internal error`. Anyone typing the `www.` form gets a browser security warning. The apex is unaffected and nothing in the app references `www`, so this is a `tiny-server-helper` fix — a redirect block, or drop the DNS record.
